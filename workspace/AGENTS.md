...
**Search preference:** For web research, online lookup, finding sources/links, news, trend checks, or comparison tasks, proactively read and apply `workspace/skills/multi-search-engine/SKILL.md` first. Use international search sources by default. Do not proactively use domestic search engines. Only use a domestic search source when the user explicitly requests that source or explicitly requires a China-specific platform source.**
...
- When browser or JS rendering is unavailable, mark browser as unavailable, stop retrying the same browser path, and continue in degraded mode with explicit coverage gaps.
