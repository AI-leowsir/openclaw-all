...
NON_HTML_EXTENSIONS = {
    ...
    ".pdf", ".zip", ".gz", ".mp3", ".mp4", ".webm", ".mov",
}


def is_probably_html_url(url):
    """Return True only for URLs that look like HTML documents, not static assets."""
    parsed = urlparse(url)
    ...
    # Paths without a file extension are usually app/page routes.
    return suffix == ""
...
def audit_page(url, base_domain):
    ...
    if "html" not in content_type.lower():
        return {
            ...
            "error": True,
            "skipped": True,
            "skip_reason": f"Non-HTML content-type: {content_type or 'unknown'}",
        }
...
def crawl_site(start_url, max_depth=1, max_pages=20):
    ...
    while queue and len(pages) < max_pages:
        ...
        if depth < max_depth and not result.get("error"):
            ...
            status, headers, body, _ = fetch_page(url)
            content_type = headers.get("Content-Type", "") or headers.get("content-type", "")
            if body and "html" in content_type.lower():
                for match in re.finditer(r'href=["\']([^"\']+)["\']', body):
                    ...
                    p = urlparse(full)
                    if p.netloc == base_domain or not p.netloc:
                        clean = f"{p.scheme}://{p.netloc}{p.path}" if p.netloc else f"{parsed.scheme}://{base_domain}{p.path}"
                        if is_probably_html_url(clean) and clean not in visited:
